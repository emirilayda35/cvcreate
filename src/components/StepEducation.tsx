...
const empty = (): Education => ({ id: crypto.randomUUID(), school: "", degree: "", field: "", startDate: "", endDate: "", gpa: "" });
...
export default function StepEducation({ data, onChange, t }: Props) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
      {data.length === 0 && <p style={{ color: "var(--color-text-muted)", fontSize: "14px", textAlign: "center", padding: "20px 0" }}>{t.noEducation}</p>}
      {data.map((edu, idx) => (
        <div key={edu.id} style={{ border: "1.5px solid var(--color-border)", borderRadius: "var(--radius-md)", overflow: "hidden" }}>
          ...
          {open === edu.id && (
            <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "14px", background: "var(--color-surface)" }}>
              <IR label={t.school} placeholder="İstanbul Teknik Üniversitesi" value={edu.school} onChange={v => update(edu.id, "school", v)} />
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                <IR label={t.degree} placeholder="Lisans" value={edu.degree} onChange={v => update(edu.id, "degree", v)} />
                <IR label={t.field} placeholder="Bilgisayar Müh." value={edu.field} onChange={v => update(edu.id, "field", v)} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px" }}>
                <IR label={t.startDate} type="month" value={edu.startDate} onChange={v => update(edu.id, "startDate", v)} />
                <IR label={t.endDate} type="month" value={edu.endDate} onChange={v => update(edu.id, "endDate", v)} />
                <IR label={t.gpa} placeholder="3.50" value={edu.gpa} onChange={v => update(edu.id, "gpa", v)} />
              </div>
...
function IR({ label, placeholder, value, onChange, type }: { label: string; placeholder?: string; value: string; onChange: (v: string) => void; type?: string }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
      <label style={LS}>{label}</label>
      <input type={type || "text"} placeholder={placeholder} value={value} onChange={e => onChange(e.target.value)} style={IS} />
    </div>
  );
}

const LS: React.CSSProperties = { fontSize: "13px", fontWeight: 500, color: "var(--color-text-body)" };
const IS: React.CSSProperties = { padding: "9px 13px", borderRadius: "var(--radius-sm)", border: "1.5px solid var(--color-border)", background: "var(--color-surface)", color: "var(--color-text-heading)", fontSize: "14px", outline: "none", width: "100%" };
