...
const empty = (): Experience => ({ id: crypto.randomUUID(), company: "", position: "", startDate: "", endDate: "", current: false, description: "" });
...
export default function StepExperience({ data, onChange, t }: Props) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
      {data.length === 0 && <p style={{ color: "var(--color-text-muted)", fontSize: "14px", textAlign: "center", padding: "20px 0" }}>{t.noExperience}</p>}
      {data.map((exp, idx) => (
        <div key={exp.id} style={{ border: "1.5px solid var(--color-border)", borderRadius: "var(--radius-md)", overflow: "hidden" }}>
          ...
          {open === exp.id && (
            <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "14px", background: "var(--color-surface)" }}>
              <IR label={t.position} placeholder="Frontend Developer" value={exp.position} onChange={v => update(exp.id, "position", v)} />
              <IR label={t.company} placeholder="Acme Corp" value={exp.company} onChange={v => update(exp.id, "company", v)} />
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                <IR label={t.startDate} type="month" value={exp.startDate} onChange={v => update(exp.id, "startDate", v)} />
                {!exp.current && <IR label={t.endDate} type="month" value={exp.endDate} onChange={v => update(exp.id, "endDate", v)} />}
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                ...
                <textarea rows={3} placeholder="..." value={exp.description} onChange={e => update(exp.id, "description", e.target.value)} style={TA} />
              </div>
